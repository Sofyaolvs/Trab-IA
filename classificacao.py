...
# le o csv e separa as 2 features e os rotulos
def carregar_dados_classificacao(filepath="EMGsDataset.csv"):
    data = np.genfromtxt(filepath, delimiter=',')

    # tenta descobrir se os dados tao em linhas ou colunas
    if data.shape[0] == 3:
        x1 = data[0, :]
        x2 = data[1, :]
...
def executar_classificacao():
    print("\n\n" + "=" * 70)
    print("PARTE 2: TAREFA DE CLASSIFICACAO")
    print("=" * 70)

    x1, x2, labels = carregar_dados_classificacao("EMGsDataset.csv")
    N = len(labels)
    C = 5  # 5 classes de expressao facial
    p = 2  # 2 sensores (features)

    print(f"\nDados carregados: {N} amostras, {p} caracteristicas, {C} classes")
    ...
    # monta X com bias pra MQO e Y em one-hot
    X_mqo = np.column_stack([np.ones(N), x1, x2])
    Y_mqo = one_hot_encode(labels, C)

    # formato do gaussiano: features nas linhas, amostras nas colunas
    X_gauss = np.vstack([x1, x2])

    print(f"\nFormato MQO:  X={X_mqo.shape}, Y={Y_mqo.shape}")
    print(f"Formato Gauss: X={X_gauss.shape}")

    # grafico de espalhamento colorido por classe
    plt.figure(figsize=(10, 7))
    cores = ['blue', 'green', 'orange', 'red', 'purple']
    for c in range(1, C + 1):
        idx = labels == c
        plt.scatter(x1[idx], x2[idx], alpha=0.3, s=5, c=cores[c-1], label=classes_nomes[c])
    plt.xlabel("Sensor 1 - Corrugador do Supercilio", fontsize=12)
    plt.ylabel("Sensor 2 - Zigomatico Maior", fontsize=12)
    plt.title("Grafico de Espalhamento - EMG Dataset", fontsize=14)
    ...
    # acha o melhor lambda usando k-fold
    print("\nExecutando K-Fold Cross Validation para encontrar l ideal...")
    ...
    for l, acc in sorted(media_acc_lambdas.items()):
        ...
        print(f"  l = {l:.3f}: Acuracia = {acc:.4f}{marcador}")

    print(f"\nl ideal = {melhor_lambda}")

    # grafico da acuracia por lambda
    ...
    plt.axvline(x=melhor_lambda, color='red', linestyle='--', label=f'l ideal = {melhor_lambda}')
    plt.xlabel("l", fontsize=12)
    plt.ylabel("Acuracia Media (K-Fold)", fontsize=12)
    plt.title("Selecao de Hiperparametro l - Classificador Regularizado (Friedman)", fontsize=13)
    plt.legend(fontsize=11)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("resultados/06_lambda_selecao.png", dpi=150)
    plt.close()
    print("Grafico de selecao de l salvo.")

    # 500 rodadas de monte carlo comparando os 6 classificadores
    R = 500
    nomes_classif = [
        "MQO Tradicional",
        ...
        "Naive Bayes",
        f"Gaussiano Regularizado (l={melhor_lambda})"
    ]
    ...
    for r in range(R):
        ...
        # dados no formato do gaussiano
        X_g_train = X_gauss[:, train_idx]
        X_g_test = X_gauss[:, test_idx]

        # estima os parametros gaussianos (media, cov, prior de cada classe)
        medias, covariancias, priors = estimar_parametros_gaussiano(X_g_train, y_train, C)

        # gaussiano tradicional (cada classe com sua propria cov)
        ...
        # gaussiano com cov pooled (mesma cov pra todas as classes)
        ...
        # gaussiano com cov agregada (media ponderada das covs)
        ...
        # naive bayes (so usa a diagonal da cov, ignora correlacao)
        ...
        # gaussiano regularizado (friedman) com o lambda ideal do k-fold
        cov_reg = {}
        for c in range(1, C + 1):
    ...
    # printa tabela de acuracia
    print("\n" + "=" * 90)
    print("RESULTADOS DA CLASSIFICACAO - ACURACIA")
    print("=" * 90)
    print(f"{'Modelo':<45} {'Media':>10} {'Desvio':>10} {'Maior':>10} {'Menor':>10}")
    print("-" * 90)
    ...
    # boxplot da acuracia de cada classificador
    ...
    plt.title("Distribuicao da Acuracia por Classificador", fontsize=14)
    ...
    # grafico de barras com media e desvio
    plt.figure(figsize=(12, 6))
    ...
    plt.title("Acuracia Media (+/- Desvio-Padrao) por Classificador", fontsize=14)
    plt.ylabel("Acuracia", fontsize=12)
    ...
    print("\nGraficos de classificacao salvos.")
