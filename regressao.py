...
def executar_regressao():
    print("=" * 70)
    print("PARTE 1: TAREFA DE REGRESSAO")
    print("=" * 70)
    ...
    plt.ylabel("Potencia Gerada", fontsize=12)
    plt.title("Grafico de Espalhamento - Aerogerador", fontsize=14)
    ...
    print("=" * 90)
    print(f"{'Modelo':<35} {'Media':>12} {'Desvio-Padrao':>15} {'Maior':>12} {'Menor':>12}")
    print("-" * 90)
    ...
    axes[0].set_title("Distribuicao do MSE por Modelo", fontsize=13)
    ...
    for patch, color in zip(bp2['boxes'], colors):
        patch.set_facecolor(color)
    axes[1].set_title("Distribuicao do R2 por Modelo", fontsize=13)
    axes[1].set_ylabel("R2", fontsize=11)
    ...
    axes[0].set_title("MSE Medio (+/- Desvio-Padrao)", fontsize=13)
    axes[0].set_ylabel("MSE", fontsize=11)
    ...
    axes[1].set_title("R2 Medio (+/- Desvio-Padrao)", fontsize=13)
    axes[1].set_ylabel("R2", fontsize=11)
    ...
    plt.axhline(y=np.mean(y_raw), color='green', linestyle='--', linewidth=2, label='Media')

    plt.xlabel("Velocidade do Vento", fontsize=12)
    plt.ylabel("Potencia Gerada", fontsize=12)
    plt.title("Modelos de Regressao - Ajuste aos Dados", fontsize=14)
    plt.legend(fontsize=11)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("resultados/04_ajuste_regressao.png", dpi=150)
    plt.close()
    print("Grafico de ajuste salvo.")
